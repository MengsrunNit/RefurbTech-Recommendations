...
import { computeRatio, monthsBetween, senseCurrentValue as senseIphone } from '../models/iphonePricingModel.js';
...
const loadPhones = async () => {
    ...
    allPhonesCache = phones.map(p => {
        ...
        const hasTelephoto = cameras.some(c => c.type === 'telephoto' || (c.details && c.details.toLowerCase().includes('telephoto')));
        ...
        const baseStorage = storageOptions.length > 0 ? Math.min(...storageOptions) : 128;
            ...
            try {
                ...
                if (fitModelKey === 'pixel') {
                    ...
                    if (fitModelKey.startsWith('iphone')) {
                        ...
                        if (key) {
                            if (key.includes('pro_max')) resolvedLaunch = resolveIphoneProMaxLaunch(key, baseStorage);
                            else if (key.includes('pro')) resolvedLaunch = resolveIphoneProLaunch(key, baseStorage);
                            else resolvedLaunch = resolveIphoneBaseLaunch(key, baseStorage);
                        }
                        
                        if (p.title.toLowerCase().includes('iphone 14 pro')) {
                             throw new Error(`DEBUG: ${p.title} -> Key: ${key}, BaseStorage: ${baseStorage}, Resolved: ${resolvedLaunch}, OldLaunch: ${launchPrice}`);
                        }
...
const calculateScore = (phone, survey) => {
    ...
    let minYear = 2018;
    if (survey.longevity === '2_3_years') minYear = 2020;
    if (survey.longevity === '4_plus_years') minYear = 2022;
    ...
    if (survey.usage && survey.usage.length > 0) {
        ...
        if (survey.usage.includes('heavy_gaming')) {
            ...
            const isPro = n.modelName.toLowerCase().includes('pro') || n.modelName.toLowerCase().includes('ultra');
    ...
    if (survey.screenSize) {
        if (survey.screenSize === 'compact' && n.screenSize < 6.1) score += 20;
        else if (survey.screenSize === 'compact' && n.screenSize >= 6.1) score -= 10;

        if (survey.screenSize === 'standard' && n.screenSize >= 6.1 && n.screenSize <= 6.6) score += 20;
        
        if (survey.screenSize === 'large' && n.screenSize > 6.6) score += 20;
        else if (survey.screenSize === 'large' && n.screenSize <= 6.6) score -= 10;
    }

    // 5. STORAGE
    if (survey.storage) {
        const required = parseInt(survey.storage, 10);
        if (!isNaN(required)) {
            const hasStorage = n.storage.some(s => s >= required);
            if (hasStorage) score += 10;
            else score -= 50; // Penalize if it doesn't have the option
        }
    }
    ...
    if (n.modelName.toLowerCase().includes('pro') || n.modelName.toLowerCase().includes('ultra')) {
        score += 5;
...
export const getRecommendationsLogic = async (survey) => {
    ...
    if (!survey.brand) {
        const alternativeBrandMatch = validMatches.find(p => !brandsIncluded.has(p.phone.brand));
